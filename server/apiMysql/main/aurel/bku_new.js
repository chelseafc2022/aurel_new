...
router.post('/view', (req, res) => {
    ...
    if (req.body.unit_kerja && req.body.unit_kerja !== '') {
        ...
        unit_filter = ` AND bku_new.unit_kerja = '${req.body.unit_kerja}' `;
    }

    var tahun_filter = '';
    if (req.body.tahun) {
        tahun_filter = ` AND bku_new.tahun = '${req.body.tahun}' `;
    }
    ...
    let jml_data_query = `
        SELECT bku_new.id
        FROM bku_new
        LEFT JOIN simpeg.unit_kerja uk ON bku_new.unit_kerja = uk.id
        WHERE (bku_new.nama_file LIKE '%${cari}%' OR bku_new.keterangan LIKE '%${cari}%')
        ${unit_filter}
        ${tahun_filter}
    `;
    ...
    let view_query = `
        SELECT bku_new.*, uk.unit_kerja as unit_kerja_uraian
        FROM bku_new
        LEFT JOIN simpeg.unit_kerja uk ON bku_new.unit_kerja = uk.id
        WHERE (bku_new.nama_file LIKE '%${cari}%' OR bku_new.keterangan LIKE '%${cari}%')
        ${unit_filter}
        ${tahun_filter}
        ORDER BY bku_new.createAt DESC
...
router.post('/hapusData', (req, res) => {
    ...
    let del_query = `DELETE FROM bku_new WHERE id = '${id}'`;
