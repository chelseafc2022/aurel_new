...
router.post('/view', (req, res) => {
    ...
    if (req.body.unit_kerja && req.body.unit_kerja !== '') {
        unit_filter = ` AND lpj.unit_kerja = '${req.body.unit_kerja}' `;
    }

    var tahun_filter = '';
    if (req.body.tahun) {
        tahun_filter = ` AND lpj.tahun = '${req.body.tahun}' `;
    }

    let jml_data_query = `
        SELECT lpj.id
        FROM lpj
        LEFT JOIN simpeg.unit_kerja uk ON lpj.unit_kerja = uk.id
        WHERE (lpj.nama_file LIKE '%${cari}%' OR lpj.keterangan LIKE '%${cari}%')
        ${unit_filter}
        ${tahun_filter}
    `;

    let view_query = `
        SELECT lpj.*, uk.unit_kerja as unit_kerja_uraian
        FROM lpj
        LEFT JOIN simpeg.unit_kerja uk ON lpj.unit_kerja = uk.id
        WHERE (lpj.nama_file LIKE '%${cari}%' OR lpj.keterangan LIKE '%${cari}%')
        ${unit_filter}
        ${tahun_filter}
        ORDER BY lpj.createAt DESC
...
router.post('/hapusData', (req, res) => {
    ...
    let del_query = `DELETE FROM lpj WHERE id = '${id}'`;
