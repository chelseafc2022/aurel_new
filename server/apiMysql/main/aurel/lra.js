...
router.post('/view', (req, res) => {
    ...
    if (req.body.unit_kerja && req.body.unit_kerja !== '') {
        unit_filter = ` AND lra.unit_kerja = '${req.body.unit_kerja}' `;
    }

    var tahun_filter = '';
    if (req.body.tahun) {
        tahun_filter = ` AND lra.tahun = '${req.body.tahun}' `;
    }

    let jml_data_query = `
        SELECT lra.id
        FROM lra
        LEFT JOIN simpeg.unit_kerja uk ON lra.unit_kerja = uk.id
        WHERE (lra.nama_file LIKE '%${cari}%' OR lra.keterangan LIKE '%${cari}%')
        ${unit_filter}
        ${tahun_filter}
    `;

    let view_query = `
        SELECT lra.*, uk.unit_kerja as unit_kerja_uraian
        FROM lra
        LEFT JOIN simpeg.unit_kerja uk ON lra.unit_kerja = uk.id
        WHERE (lra.nama_file LIKE '%${cari}%' OR lra.keterangan LIKE '%${cari}%')
        ${unit_filter}
        ${tahun_filter}
        ORDER BY lra.createAt DESC
...
router.post('/hapusData', (req, res) => {
    ...
    let del_query = `DELETE FROM lra WHERE id = '${id}'`;
