        ...
        if (err) { res.json(err) }
            ...
            for (let i = 0; i < result.length; i++) {
                ...
                if (jml > 0) {
                    ...
                    result[i].jml = result[i].nilai_ukpbj_doc || 0
                  }
...
const getSubKegiatanRincianNew = async (kegiatan_sub_kode, tahun) => {
    if (!tahun) {
      // tahun tidak ada → jangan ambil apa pun
      return []
    }
      ...
      let q = `
        SELECT 
          ...
          ( SELECT SUM(realisasi)
            ...
            WHERE realisasi_keuangan_serapan.rincian_id = r.id
          ) AS total_jml_realisasi_keuangan
        FROM kegiatan_sub_rincian_new r
...
const getSubKegiatan = async (kegiatan_kode, req) => {
    return new Promise((resolve, reject) => {
      let q = `
        ...
        WHERE kegiatan_sub.kegiatan_kode = '`+kegiatan_kode+`'
          AND kegiatan_sub.unit_kerja = '`+req.body.unit_kerja+`'
      `;
  
      db.query(q, async (err, row) => {
        if (err) {
          console.log(err)
          resolve([])
        } else {
          ...
          for (let i = 0; i < row.length; i++) {
            const rincian = await getSubKegiatanRincianNew(
                row[i].kode_full,
                req.body.tahun
              )
            ...
            rincian.forEach(r => {
            jml += r.jml
            jml_realisasi += r.total_jml_realisasi_keuangan || 0
            })
