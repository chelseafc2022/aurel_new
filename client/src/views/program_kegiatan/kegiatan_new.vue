                ...
                <template v-for="(data, index) in list_data">
                  ...
                  <template v-for="(data1, index1) in data.sub_kegiatan">
                    ...
                    <tr class="h_table_body bg-white" :key="'sub-'+data1.id">
                      ...
                      <td class="text-center">
                        <q-btn-dropdown round dense color="purple-4" icon="pending">
                          <q-list>
                            <q-item clickable v-close-popup @click="openTambahRincian(data1)">
                              <q-item-section>
                    ...
                    <tr
                      v-if="data1.rincian && data1.rincian.length"
                      class="h_table_head1 softGrey"
                    >
                      <th></th>
                      <th></th>
                      <th>Uraian</th>
                    ...
                    <tr
                      v-for="r in data1.rincian"
                      :key="'rincian-'+r.id"
                      class="h_table_body bg-white"
                    >
                      <td></td>
                      <td></td>
                      <td>{{ r.uraian }}</td>
                      <td>{{ r.koefisien }}</td>
                      <td>{{ r.satuan }}</td>
                      <td>{{ UMUM.Rupiah(r.harga) }}</td>
                      <td>{{ r.ppn }}</td>
                      <td>{{ UMUM.Rupiah(r.jml) }}</td>
                      <td class="text-center">
                        <q-btn-dropdown round dense color="orange-3" icon="pending">
                          <q-list>
                            <q-item clickable v-close-popup @click="selectRincian(r); mdl_edit_sub_kegiatan_rincian = true">
                              <q-item-section>
              ...
              <q-dialog v-model="mdl_hapus_sub_kegiatan_rincian" persistent>
                  ...
                  <q-card-section class="q-pt-none text-center orageGrad">
                      <form @submit.prevent="removeRincian()">
                          ...
                          <br>
                          <span class="h_notifikasi">APAKAH ANDA YAKIN INGIN MENGHAPUS RINCIAN DATA INI??</span>
                          <input type="submit" style="position: absolute; left: -9999px"/>
                          <br>
                          <br>
    ...
    methods: {
      ...
      addDataKegiatanSub : function(){
        fetch(this.$store.state.url.CT_KEGIATAN_SUB + "addData", {
            ...
            this.Notify('Sukses Menambah Data', 'primary', 'check_circle_outline');
            this.rincian.kegiatan_sub_id   = res_data.kegiatan_sub_id
            this.rincian.kegiatan_sub_kode = res_data.kode_full
            this.mdl_add_sub_kegiatan = false
            this.mdl_add_sub_kegiatan_rincian = true
            });
      },
      ...
      openTambahRincian (sub) {
        this.rincian = {
          id: '',
          kegiatan_sub_id: sub.kegiatan_sub_id,
          kegiatan_sub_kode: sub.kode_full,
          uraian: '',
          koefisien: '',
          satuan: '',
          harga: 0,
      ...
      openHapusRincian (r) {
        this.rincian = { ...r }
        this.mdl_hapus_sub_kegiatan_rincian = true
      },
  
      addDRincian () {
        fetch(this.$store.state.url.CT_KEGIATAN_SUB_RINCIAN_NEW + "addData", {
          method: "POST",
          headers: {
            "content-type": "application/json",
            authorization: "kikensbatara " + localStorage.token
          ...
          body: JSON.stringify({
            kegiatan_sub_kode: this.rincian.kegiatan_sub_kode,
            kegiatan_sub_id: this.rincian.kegiatan_sub_id,
            uraian: this.rincian.uraian,
            koefisien: this.rincian.koefisien,
            satuan: this.rincian.satuan,
            harga: this.rincian.harga,
            ppn: this.rincian.ppn,
            jml: this.rincian.jml
          })
        }).then(() => {
          this.Notify('Rincian berhasil ditambahkan', 'primary', 'check_circle_outline')
          this.getView()
        })
      },
      ...
      editRincian : function(){
        fetch(this.$store.state.url.CT_KEGIATAN_SUB_RINCIAN_NEW + "editData", {
            method: "POST",
            headers: {
              "content-type": "application/json",
              authorization: "kikensbatara " + localStorage.token
      ...
      removeRincian : function(E){
        fetch(this.$store.state.url.CT_KEGIATAN_SUB_RINCIAN_NEW + "removeData", {
            method: "POST",
            headers: {
              "content-type": "application/json",
              authorization: "kikensbatara " + localStorage.token
